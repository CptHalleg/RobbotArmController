...
namespace IngameScript {
    internal class PARK : MyGridProgram {
        /*
...
*/
        ...
        */
        ...
        readonly bool toggleRotorPower = false;// This can reduce the amount of wobbling that occurs when rotors are set to high torque.
        ...
        public void Main(string argument, UpdateType updateSource) {
            ...
            if ((updateSource & UpdateType.IGC) != 0) {// Alter the MoveFlag according to the ushort.
            ...
            if (returnSeq != -1) {
                ...
                foreach (Motor m in returningMotors) {
                    isReturning |= !m.MotorReturn(multiplier, profile, returnDeadZone, toggleRotorPower);
            ...
            if (rampTicks != 0)// Filter to ensure the script only sends updates to the blocks when the input changes.
            {
                if (motors.Count > 0) {
                    foreach (Motor motor in motors) {
                        if (motor.Actuate(moveFlagShort, profile, multiplier / rampTicks, returnDeadZone, toggleRotorPower)) {
            ...
            foreach (Motor m in autoReturningMotors) {
                autoReturn |= !m.MotorReturn(multiplier, profile, returnDeadZone, toggleRotorPower);
            }
            if (!autoReturn)
        ...
        string lastDebug = "";
        void RenderScreen(string DebugStr = "") {
        ...
        public class Motor : Controllable {
            ...
            public bool Actuate(ushort flag, int profile, float multiplier, float deadZone, bool controlPower) {
                ...
                if (flag == 0) {
                    if (returnOrder == -1) {
                        MotorReturn(multiplier, profile, deadZone, controlPower);
                        return true;
                    }
                    if (controlPower)
                        motor.Enabled = false;
            ...
            /// </summary>
            public bool MotorReturn(float multiplier, int profile, float deadZone, bool controlPower) {
